...
#if defined(_OPENMP) &&                                                        \
    __has_include(                                                             \
        <hip/hip_runtime_api.h>) && (defined(__HIP_PLATFORM_AMD__) || defined(__HIP_PLATFORM_NVIDIA__))
#include <hip/hip_runtime_api.h>
#include <omp.h>

// Define Overlays for HIP host functions
__attribute__((weak)) hipError_t _ovlh_hipHostMalloc(void **p, size_t sz,
                                                     unsigned int flags) {
...
__attribute__((weak)) hipError_t hipHostMalloc(void **p, size_t sz,
                                               unsigned int flags) {
  return _ovlh_hipHostMalloc(p, sz, flags);
}

// Calls to hipMalloc will register memory with omp runtime
__attribute__((weak)) hipError_t hipMalloc(void **p, size_t sz) {
  return _ovlh_hipMalloc(p, sz);
}
